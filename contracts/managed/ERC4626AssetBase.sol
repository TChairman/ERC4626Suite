...
abstract contract ERC4626AssetBase is ERC4626SuiteContext {
    ...
    // Events
    event createAssetEvent(bytes32 indexed _type, address indexed _vault, uint256 _reference, uint256 _amount, uint256 _expectedReturnBPS);
    ...
    constructor() {
        assetList.push(assetStruct(0x00, address(0), 0, 0, 0, 0)); // create guard entry in assetList;
    }
    ...
    function createAsset(
        ...
        assetList.push(assetStruct(_assetType, _assetAddress, _assetReference, 0, block.timestamp, 0));
        if (_assetNAV > 0) setNAVindex(index, _assetNAV);
        if (_expectedReturnBPS > 0) setExpectedReturnBPSindex(index, _expectedReturnBPS);
        emit createAssetEvent(_assetType, _assetAddress, _assetReference, _assetNAV, _expectedReturnBPS);
        return true;
    }
    ...
    function deleteAsset(bytes32 _assetType, address _assetAddress, uint256 _assetReference) public virtual onlyManager {
        ...
        if (assetList[index].assetNAV > 0) setNAVindex(index, 0);
        ...
        if (index < assetList.length-1) {
            ...
            assetIndexes[assetHash(assetList[index].assetAddress, assetList[index].assetReference)] = index;
        }
        assetList.pop();
    ...
    function getLatestNAV(assetStruct storage asset) internal view virtual returns (uint256) {
        return asset.assetNAV.mulDiv(BPS_MULTIPLE + asset.expectedReturnBPS, BPS_MULTIPLE);
    }
    ...
    function setNAVindex(uint32 index, uint256 newNAV) internal virtual {
        require(index < assetList.length, "Index out of range");
        uint256 oldNAV = assetList[index].assetNAV;
        assetList[index].assetNAV = newNAV;
    ...
    function setExpectedReturnBPSindex(uint32 index, uint256 newBPS) internal virtual {
        ...
        if (newBPS != oldBPS) {
            ...
            totalExpectedReturnPerSec = totalExpectedReturnPerSec + 
                            newBPS.mulDiv(assetList[index].assetNAV, BPS_MULTIPLE * SECS_PER_YEAR) -
                            oldBPS.mulDiv(assetList[index].assetNAV, BPS_MULTIPLE * SECS_PER_YEAR);
            emit setExpectedReturnBPSEvent(assetList[index].assetType, assetList[index].assetAddress, assetList[index].assetReference, oldBPS, newBPS);
        }
    }
    ...
    function updateAllAssets() public virtual returns (uint256) {
        ...
        for(uint32 i=0; i<= assetList.length; i++){
            (assetList[i].assetNAV,  assetList[i].expectedReturnBPS) = getLatestNAVandReturnBPS(assetList[i]);
            totalAssetNAV += assetList[i].assetNAV;
            totalExpectedReturnPerSec += assetList[i].expectedReturnBPS.mulDiv(assetList[i].assetNAV, BPS_MULTIPLE * SECS_PER_YEAR);
        }
